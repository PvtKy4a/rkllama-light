from rkllm_types import *
from models_utils import *

# Connect the callback function between the Python side and the C side
rkllm_cb_type = ctypes.CFUNCTYPE(None, ctypes.POINTER(RKLLMResult), ctypes.c_void_p, ctypes.c_int)
...
class RKLLMModel:
    def __init__(self, lib_path, models_path, model_name):
        ...
        self.rkllm_init.argtypes = [ctypes.POINTER(RKLLM_Handle_t), ctypes.POINTER(RKLLMParam), rkllm_cb_type]
        self.rkllm_init(self.handle, self.rkllm_param, self.rkllm_cb)
        ...
        self.rkllm_run.argtypes = [RKLLM_Handle_t, ctypes.POINTER(RKLLMInput), ctypes.POINTER(RKLLMInferParam), ctypes.py_object]
    ...
    def list_to_ctypes_array(self, tokens, ctype):
    ...
    def RKLLM_run(self, request, regenerate=False, enable_history=True):
        ...
        self.rkllm_input.input_data.token_input.input_ids = self.list_to_ctypes_array(tokens, ctypes.c_int)

        self.rkllm_input.input_data.token_input.n_tokens = ctypes.c_ulong(len(tokens))

        self.rkllm_run(self.handle, ctypes.byref(self.rkllm_input), ctypes.byref(self.rkllm_infer_params), ctypes.py_object(self))
